...
class BayesianOptimizer:
    ...
    def _evaluate_candidate(
        ...
        param_dict = dict(zip(self.param_names, candidate_values, strict=False))
        combined_params = {**self.fixed_params, **param_dict}

        # 1. Generate GDSFactory component
        t_geom_0 = time.perf_counter()
        component = self.cell_factory(**combined_params)
        ...
        if self.enforce_connectivity:
            is_conn, _n_comp, msg = check_slab_connectivity(
                ...
                check_pbc=True,
                min_neck_width_px=self.min_neck_width_px,
            )
            if not is_conn:
                ...
                record = OptimizationRecord(
                    ...
                    cost=1.0,
                    fom=1.0,
                    status=f"FAILED: Disconnected dielectric ({msg})",
                    connectivity=conn_status,
                    timing={"t_geom": t_geom, "t_solver": 0.0, "t_total": t_total},
        ...
        enriched_meta["solver_data"] = _extract_solver_data(
            solver_res, pol_key=pol, num_bands=self.num_bands
        )
    ...
    def run(self, show_progress: bool | None = None) -> OptimizationResult:
        ...
        try:
            while curr_eval < total_evals:
                ...
                batch_records: list[OptimizationRecord] = []
                batch_costs: list[float] = []

                for x_cand in candidates:
                    rec = self._evaluate_candidate(x_cand, gen=gen)
